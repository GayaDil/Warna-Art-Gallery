...
$save_btn = '<a href="javascript:void(0);" class="btn btn-info add-artwork">SAVE</a>';
...
if ( isset($_GET['id'])) {
  ...
  $image = $prd->product($get_id)['image_name']; //path
  ...
  $save_btn = '<a href="javascript:void(0);" class="btn btn-info update-artwork">SAVE</a>';
  ...
  //Redirect if this product not belong to this user
...
//List all mediums
$medium_list = '';
$query = $db->query("SELECT * FROM `mediums` WHERE `status` = 1 ");
$rowCount = $query->num_rows;
if($rowCount > 0){
  while($row = $query->fetch_assoc()){
    $id = $row['id'];
    $type = $row['type'];
    $selected = ( $id == $medium_id ) ? 'selected' : '';
